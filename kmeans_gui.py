...
def run_kmeans_analysis(config, output_text):
    ...
    def log(msg, level='INFO'):
        ...
        if output_text:
            output_text.append(f"[{level}] {msg}\n")
    
    log("=" * 80)
    log("K-MEANS CLUSTERING - ANÁLISIS")
    log("=" * 80)
    log(f"Dataset: {n} observaciones, {X_raw.shape[1]} dimensiones")
    
    # Determinar K
    if config['auto_k']:
        log("Buscando K óptimo...")
        ...
        config['K'] = optimal_k
        log(f"K óptimo encontrado: {optimal_k} (basado en Silhouette Score)")
    else:
        config['K'] = int(config['K'])
        log(f"K manual: {config['K']}")
    
    log(f"Configuración: K={config['K']}, init='{config['init_method']}', "
        f"scale={config['scale_data']}, random_state={config['random_state']}")
    ...
    if config['detect_outliers']:
        ...
        log(f"Método: {actual_method.upper()}")
        log(f"Outliers detectados: {n_outliers}/{n}")
    
    # 3. INICIALIZACIÓN
    log("=" * 80)
    log("INICIALIZACIÓN DE CENTROIDES")
    log("=" * 80)
    ...
    if config['init_method'] == 'farthest':
        ...
        n_init = 1
        log("Método: Farthest Point")
    elif config['init_method'] == 'kmeans++':
        initial_centroids = 'k-means++'
        n_init = 10
        log("Método: K-means++ (sklearn, 10 reinicios)")
    else:
        initial_centroids = X[:config['K']].copy()
        n_init = 1
        log(f"Método: Primeros {config['K']} puntos")
    
    # 4. EJECUTAR K-MEANS
    log("=" * 80)
    log("EJECUTANDO K-MEANS")
    log("=" * 80)
    ...
    kmeans.fit(X)
    log(f"✓ Convergencia en {kmeans.n_iter_} iteraciones")
    log(f"Inercia: {kmeans.inertia_:.6f}")
    
    # 5. VALIDACIÓN
    log("=" * 80)
    log("VALIDACIÓN DE CLUSTERING")
    log("=" * 80)
    ...
    if issues:
        ...
        log("✓ No se detectaron problemas de validación")
    ...
    if len(unique_labels) > 1:
        ...
        log(f"Silhouette Score: {silhouette:.4f} (mejor: → 1)")
        log(f"Davies-Bouldin Index: {davies_bouldin:.4f} (mejor: → 0)")
        log(f"Calinski-Harabasz Index: {calinski:.4f} (mejor: ↑)")
    else:
    ...
    for label, count in zip(unique_labels, counts):
        log(f"  Cluster {label}: {count} puntos ({count/n*100:.1f}%)")
    
    # 7. COMPARACIÓN CON GROUND TRUTH
    log("=" * 80)
    log("COMPARACIÓN CON CLASES VERDADERAS")
    log("=" * 80)
    ...
    if mapped_labels is not None:
        log(f"Mapeo encontrado: {mapping}")
        accuracy = np.sum(mapped_labels == true_labels) / n * 100
        log(f"Exactitud: {np.sum(mapped_labels == true_labels)}/{n} = {accuracy:.2f}%")
        ...
        log("No se puede calcular accuracy (K_pred ≠ K_true)", 'WARNING')
        ...
        for pred_label in np.unique(kmeans.labels_):
            ...
            for true_label in np.unique(true_labels):
                ...
                if count > 0:
                    log(f"  Clase {true_label}: {count} puntos")
    ...
    for k in range(results['config']['K']):
        ...
        if np.any(mask):
            ax1.scatter(results['X_raw'][mask, 0], results['X_raw'][mask, 1], 
                      c=[colors_map[k]], label=f'Cluster {k}', s=120, alpha=0.7, 
...
def show_info_window():
    ...
    section1 = ttk.LabelFrame(content, text='Selección de K', padding="10")
    section1.pack(fill=tk.X, pady=5)
    
    ttk.Label(section1, text='• Automático (basado en Silhouette):', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=2)
    ttk.Label(section1, 
             text='El algoritmo busca automáticamente el número óptimo de clusters (K) evaluando '
             ...
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    ttk.Label(section1, text='• Manual:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=(10, 2))
    ttk.Label(section1, 
             text='Permite especificar manualmente el número de clusters. Útil cuando se conoce '
                  'el número esperado de grupos en los datos.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    # Sección: Opciones
    section2 = ttk.LabelFrame(content, text='Opciones de Procesamiento', padding="10")
    section2.pack(fill=tk.X, pady=5)
    
    ttk.Label(section2, text='• Escalar datos:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=2)
    ttk.Label(section2, 
             text='Normaliza los datos usando StandardScaler (media=0, desviación=1). Es recomendado '
                  'cuando las características tienen diferentes escalas o unidades. Mejora la '
                  'convergencia del algoritmo K-means.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    ttk.Label(section2, text='• Detectar outliers:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=(10, 2))
    ttk.Label(section2, 
             text='Identifica puntos anómalos que pueden afectar el clustering. Los outliers se '
                  'muestran en la visualización con un círculo rojo.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    ttk.Label(section2, text='• Método de detección de outliers:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=(10, 2))
    ttk.Label(section2, 
             text='- IQR (Interquartile Range): Usa el rango intercuartílico para identificar valores '
    ...
    section3 = ttk.LabelFrame(content, text='Métodos de Inicialización', padding="10")
    section3.pack(fill=tk.X, pady=5)
    
    ttk.Label(section3, text='• kmeans++:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=2)
    ttk.Label(section3, 
             text='Método inteligente que selecciona los centroides iniciales de forma que estén '
                  'bien distribuidos. Reduce la probabilidad de convergencia a mínimos locales. '
                  'Recomendado para la mayoría de casos.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    ttk.Label(section3, text='• farthest:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=(10, 2))
    ttk.Label(section3, 
             text='Selecciona los centroides iniciales como los puntos más alejados entre sí. '
                  'Útil cuando se quiere garantizar una buena separación inicial.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    ttk.Label(section3, text='• first_k:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=(10, 2))
    ttk.Label(section3, 
             text='Usa los primeros K puntos del dataset como centroides iniciales. Método simple '
                  'pero puede llevar a resultados subóptimos.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    ttk.Label(section3, text='• Random State:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=(10, 2))
    ttk.Label(section3, 
             text='Semilla para la generación de números aleatorios. Usar el mismo valor garantiza '
                  'resultados reproducibles.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    # Sección: Métricas
    section4 = ttk.LabelFrame(content, text='Métricas de Calidad', padding="10")
    section4.pack(fill=tk.X, pady=5)
    
    ttk.Label(section4, text='• Silhouette Score:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=2)
    ttk.Label(section4, 
             text='Mide qué tan bien separados están los clusters. Rango: [-1, 1]. Valores más '
                  'altos indican mejor separación. Ideal: cercano a 1.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    ttk.Label(section4, text='• Davies-Bouldin Index:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=(10, 2))
    ttk.Label(section4, 
             text='Mide la relación entre la dispersión dentro de los clusters y la separación '
                  'entre clusters. Valores más bajos indican mejor clustering. Ideal: cercano a 0.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    ttk.Label(section4, text='• Calinski-Harabasz Index:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=(10, 2))
    ttk.Label(section4, 
             text='Ratio de la suma de dispersiones entre clusters y dentro de clusters. Valores '
                  'más altos indican mejor clustering.',
             wraplength=650, justify=tk.LEFT).pack(anchor=tk.W, padx=20, pady=2)
    
    ttk.Label(section4, text='• Inertia:', 
             font=('Helvetica', 10, 'bold')).pack(anchor=tk.W, pady=(10, 2))
    ttk.Label(section4, 
             text='Suma de las distancias al cuadrado de cada punto a su centroide más cercano. '
    ...
    # Botón cerrar
    ttk.Button(content, text="Cerrar", command=info_window.destroy).pack(pady=10)
...
def create_gui():
    ...
    log_label = ttk.Label(left_frame, text='Log:', font=('Helvetica', 11, 'bold'))
